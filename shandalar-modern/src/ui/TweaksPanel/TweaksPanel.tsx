...
const SCENARIOS = [
  { value: 'spell-creature', label: 'Spell → Creature' },
  { value: 'spell-player', label: 'Spell → Player' },
  { value: 'creature-creature', label: 'Creature → Creature' },
  { value: 'no-arrow', label: 'No Arrow' },
];
...
export function TweaksPanel({ values, setTweak }: TweaksPanelProps) {
  ...
  // Keyboard shortcut: backtick opens panel
  useEffect(() => {
  ...
  return (
    <div ref={dragRef} className={styles.panel}
      ...
      <div className={styles.body}>
        ...
        <TweakSelect label="Arrow demo" value={values.scenario} options={SCENARIOS}
          onChange={v => setTweak('scenario', v)} />

        <TweakSection label="AI" />
        <TweakRadio label="Archetype" value={values.oppArchetype}
          options={['aggro', 'control', 'midrange']}
          onChange={v => setTweak('oppArchetype', v as TweakValues['oppArchetype'])} />
        <TweakSlider label="AI Speed" value={values.aiSpeed} min={100} max={1000} step={50} unit="ms"
          onChange={v => setTweak('aiSpeed', v)} />

        <TweakSection label="Seed" />
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <TweakNumber label="Seed" value={values.seed} min={0} max={99999}
            onChange={v => setTweak('seed', v)} />
          <TweakButton label="Reroll" secondary
            onClick={() => setTweak('seed', Math.floor(Math.random() * 99999))} />
        </div>
      </div>
    </div>
  );
