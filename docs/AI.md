...
# 12. Gemini LLM Opponent Integration

## 12.1 Module Overview

`src/engine/GeminiAdvisor.js` is the async module responsible for querying the Gemini LLM
during the opponent's turn. It is caller-side only and must never be imported by `DuelCore.js`
or `AI.js`. On any failure it returns `null`; callers must always fall back to the heuristic AI.

## 12.2 Per-Mage System Prompts

`src/engine/geminiPrompts.js` is a pure sibling module (no GenAI client dependency) that
owns the base system instruction and per-mage personality table.

### Base instruction (`SYSTEM_INSTRUCTION`)

Shared by all opponents not in `MAGE_PROMPTS`. Instructs Gemini to play competitive
Magic: The Gathering under Alpha/Beta rules, select an action index from `legalActions`,
and prioritize threat removal, board advancement, and lethal damage.

### Per-mage table (`MAGE_PROMPTS`)

Keyed by `ARCHETYPES[oppArchKey].profileId`. Each entry extends `SYSTEM_INSTRUCTION`
with a short personality and strategic bias:

| profileId | Personality |
|-----------|-------------|
| `DELENIA` | White aggro-control: deploy early, protect creatures, clear blockers, push damage |
| `XYLOS` | Blue control: hold counterspells, trade resources favorably, win the long game |
| `MORTIS` | Black attrition: trade life for advantage, one-for-one removal, resource denial |

Remaining bosses (KARAG, SYLVARA, GENERIC, ARZAKON) use the base instruction until a
future expansion pass.

### Selector (`selectSystemInstruction(profileId)`)

Pure, total, never throws. Returns the mage-specific instruction for a known `profileId`
string, or `SYSTEM_INSTRUCTION` for any unknown id, null, undefined, or non-string input.
This degradation is intentional -- a missing profile must not break the opponent's turn.

## 12.3 Call-site wiring (`useDuelController.ts`)

Resolves `oppProfileId = ARCHETYPES[config.oppArchKey]?.profileId ?? null` immediately
before calling `fetchGeminiMove(serializedState, oppProfileId)`. Non-boss opponents whose
archetype has no `profileId` yield `null`, which falls back to the base instruction.
