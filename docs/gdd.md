...
| B26 | AI plays land but casts no spells | `planMain` computed `totalMana` once before the land PLAY_CARD was queued; never updated `virtualState` after queuing the land; `card.cmc > totalMana` filtered all spells when AI had 0 existing lands | Updated `virtualState`, `availMana`, and `totalMana` immediately after queuing the land action in the land branch of `planMain` |
| B27 | AI never casts spells (all random encounter opponents) | Two stacked bugs: (1) random encounter archetypes missing `profileId` field — all resolved to GENERIC profile; (2) score gate `score * greedySpells < 0.35` in `evaluateAndCast` blocks spells for all profiles with `greedySpells < 0.7`, which includes GENERIC, DELENIA, XYLOS, and SYLVARA | Added `profileId` to all 10 random encounter archetypes in `cards.js`; lowered score gate from `0.35` to `0.10` in `AI.js` |
