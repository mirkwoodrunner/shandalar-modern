...
| B-NEW | AI never casts spells on turns it also plays a land | `planMain` queued a land-play action but did not update `virtualState` to reflect the land on the battlefield. Downstream `selectPlayableCards(virtualState, phase)` called `computeAvailableMana(virtualState)` on the stale state, returning `totalMana` that was 1 short; all spells failed the `card.cmc > totalManaCeiling` guard on those turns. | After queueing the land-play action, mirror the land into `virtualState` (`hand` filtered, land pushed to `bf` with `tapped: false`, `landsPlayed: 1`). `selectPlayableCards` then sees the correct mana pool. Action type for non-land spells (`CAST_SPELL` via `aiDecide` adapter) confirmed correct. |
