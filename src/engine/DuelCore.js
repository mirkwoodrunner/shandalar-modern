...
export function parseMana(cost, xValue = 0) {
...
else if (ch === “X”)       { p.generic += xValue; i++; }
...
export function canPay(pool, cost, xValue = 0) {
const r = parseMana(cost, xValue);
const a = { …pool };
...
export function payMana(pool, cost, xValue = 0) {
const r = parseMana(cost, xValue);
const p = { …pool };
...
case "CAST_SPELL": {
  ...
  const xVal = action.xVal || 0;
  if (!canPay(s[w].mana, c.cost, xVal)) return s;
  ...
  s = { ...s, [w]: { ...s[w], mana: payMana(s[w].mana, c.cost, xVal), hand: s[w].hand.filter(x => x.iid !== action.iid) } };
